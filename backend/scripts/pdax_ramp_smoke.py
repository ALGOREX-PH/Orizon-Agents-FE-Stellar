...
async def main() -> None:
    ...
    advanced = await ramp.handle_event(
        fake,
        FiatEvent(
            user_id="u",
            identifier="onramp-1",
    ...
    print(
        "on-ramp settled: order",
        advanced.order_id,
        "tx",
        advanced.crypto_tx_id,
        f"({advanced.usdc_amount} USDCXLM → {advanced.stellar_address})",
    )

    # ── off-ramp: USDCXLM → PHP ──
    rec2 = await ramp.start_offramp(
        fake,
        OffRampRequest(
            usdc_amount="17.18",
            identifier="offramp-1",
            beneficiary_bank_code="BAUBPPH",
            beneficiary_account_name="Juan Cruz",
            beneficiary_account_number="00123456789",
            sender_first_name="Juan",
            sender_last_name="Cruz",
            beneficiary_first_name="Juan",
            beneficiary_last_name="Cruz",
    ...
    advanced2 = await ramp.handle_event(
        fake,
        CryptoEvent(
            user_id="u",
            transaction_type="DEPOSIT",
            ...
            amount=17.18,
            asset="USDCXLM",
            destination_address="GDEPOSITADDR",
        ),
    )
    assert advanced2 and advanced2.status == "completed", advanced2
    assert advanced2.withdraw_request_id == "wreq1"
    print("off-ramp settled: order", advanced2.order_id, f"({advanced2.usdc_amount} USDC → {advanced2.php_amount} PHP)")

    # ── reconcile fallback: settle without a webhook (e.g. staging) ──
    rec3 = await ramp.start_onramp(
        fake,
        OnRampRequest(
            php_amount="200",
            stellar_address="GBUYERWALLET",
            method="instapay_upay_cashin",
            identifier="reconcile-1",
            sender_first_name="Juan",
            sender_last_name="Cruz",
            beneficiary_first_name="Juan",
            beneficiary_last_name="Cruz",
    ...
    print(
        f"funding quote: pay ₱{fq.php_to_pay} to fund {fq.usdc_target} USDC (base ₱{fq.php_base} + {fq.buffer_bps}bps)"
    )
